...
cd packages/rethink-billing/e2e/features
  if [[ "$OSTYPE" == "msys"* ]]; then
...
-F "file=@packages/rethink-billing/e2e/features/features.zip" \
-F "testInfo=@packages/rethink-billing/xrayIntegration/testInfo.json" \
"$BASE_URL/api/v2/import/feature?projectKey=$PROJECT"

rm -rf packages/rethink-billing/e2e/features/features.zip
